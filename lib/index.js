...
function concurrent(concurrency, promiseFuns) {
    ...
    if (concurrency >= promiseFuns.length)
        return Promise.all(
            promiseFuns.map(function (fun) {
                return fun();
            }),
        );
    return Promise.all(
        Array.from(new Array(concurrency), function (_, i) {
            return sequence(
                promiseFuns.filter(function (x, j) {
                    return j % concurrency === i;
                }),
            );
        }),
    ).then(function (arrayOfArrays) {
        return Array.from(new Array(promiseFuns.length), function (_, i) {
            return arrayOfArrays[i % concurrency][(i - (i % concurrency)) / concurrency];
        });
    });
}
